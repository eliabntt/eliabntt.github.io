...
class BlogBuilder {
  ...
  addVideo = (props, title) => {
    this.list.push(
      <center>
        <div className="container-fluid">
    <div key={this.list.length} className="ratio ratio-16x9">
      <iframe allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen src={props} title={title} key={this.list.length}></iframe>
    </div>
    </div>
    </center>);
    return this;
  ...
  addPaper = (props) => {
    this.list.push(
      <p key={this.list.length} className="lead">
        The <i className="fas fa-scroll"/> is accessible open-source <i className="fas fa-book-open"/><a className='badge badge-dark' 
          rel='noopener noreferrer' 
          target='_blank' 
          href={props} 
          aria-label='iRotate' > here </a>{' '}
  ...
  addCitation = (props) => {
    ...
    <div className="snippet-clipboard-content position-relative">
      ...
      <clipboard-copy aria-label="Copy" className="ClipboardButton btn js-clipboard-copy m-2 p-0 tooltipped-no-delay" data-copy-feedback="Copied!" data-tooltip-direction="w" value={props} tabindex="0" role="button">
